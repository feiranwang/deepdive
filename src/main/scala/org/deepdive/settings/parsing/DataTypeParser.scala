...
object DataTypeParser extends RegexParsers {
  ...
  def TreeNodeParser = "TreeNode" ~> "(" ~> """\d+""".r ~ ("," ~> """\d+""".r) <~ ")" ^^ { 
    case n ~ m => TreeNodeType(n.toInt, m.toInt) 
  }
  def dataType = CategoricalParser | BooleanParser | RealArrayParser | TreeNodeParser //|  RealNumberParser 
}